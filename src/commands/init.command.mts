...
import type { ConfigFile } from "../models/config-file.model.mjs";
...
const initCommand: Command = new Command("init")
  .command("init")
  .description(
    "Initialize a configuration file for the Spring CLI on the current spring project",
  )
  .action(async (): Promise<void> => {
    ...
    console.log(
      chalk.green(
        `Initialization completed, the configuration file is created at ${process.cwd()}/spring-cli.yml`,
      ),
    );
  });
...
const checkIsDirectory: (directory: string) => boolean = (
  directory: string,
): boolean => {
  try {
...
const createInitFile: (
  ...
  if (failIfExist && existsFile(filePath)) {
    console.error(
      chalk.red(`The file ${fileName} already exists in the current directory`),
    );
    process.exit(1);
  }
...
export { initCommand, createInitFile };
