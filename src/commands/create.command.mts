...
import type { Pom } from "../models/pom.model.mjs";
import {
  copyFiles,
  createFolder,
  deleteFolder,
...
import {
  ...
  isValidMavenGroupId,
  isValidProjectName,
} from "../utils/validator.utils.mjs";
import {
  parseObjectToXmlString,
  parseStringToXml,
} from "../utils/xml.utils.mjs";
import {
  parseObjectToYamlString,
  parseStringToYaml,
} from "../utils/yaml.utils.mjs";
...
const createCommand: Command = new Command("create")
  ...
  .action(async (name: string): Promise<void> => {
    ...
    // =========================================
    const javaFilePattern: string = path.posix.join(
      projectPath,
      "src",
      "main",
      "java",
    ...
    const javaTestFilePattern: string = path.posix.join(
      projectPath,
      "src",
      "test",
      "java",
    ...
    const javaFiles: string[] = getJavaFiles(javaFilePattern);
    const javaTestFiles: string[] = getJavaFiles(javaTestFilePattern);
    javaFiles.push(...javaTestFiles);
    for (const file of javaFiles) {
      let fileContent: string = readFile(file);
      fileContent = fileContent.replaceAll(
        "it.theapplegeek.spring_starter_pack",
        `${groupId.replaceAll("-", "_")}.${artifactId.replaceAll("-", "_")}`,
      );
    ...
    // =========================================
    const mainApplicationPath: string = path.join(
      projectPath,
      "src",
      "main",
      "java",
    ...
    const artifactIdCamelCase: string = artifactId
      .split(/[.-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join("");
    mainApplicationContent = mainApplicationContent.replaceAll(
      "class Application",
      `class ${artifactIdCamelCase}Application`,
    );
    mainApplicationContent = mainApplicationContent.replaceAll(
      "Application.class",
      `${artifactIdCamelCase}Application.class`,
    );
    writeFile(mainApplicationPath, mainApplicationContent);
    const mainApplicationNewPath: string = path.join(
      projectPath,
      "src",
      "main",
      "java",
      ...
      "theapplegeek",
      "spring_starter_pack",
      `${artifactIdCamelCase}Application.java`,
    );
    ...
    // =========================================
    const applicationYmlPath: string = path.join(
      projectPath,
      "src",
      "main",
      "resources",
    ...
    applicationYmlContent.spring.application.name = artifactId
      .split(/[.-]/)
      .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
      .join(" ");
    const secretKeyHex: string = crypto.randomBytes(32).toString("hex");
    applicationYmlContent.application.security.jwt["secret-key"] =
      Buffer.from(secretKeyHex).toString("base64");
    ...
    const artifactIdSplit: string[] = artifactId
      .replaceAll("-", "_")
      .split(".");
    const defaultFolderPath: string = path.join(
      projectPath,
      "src",
      "main",
      "java",
    ...
    const defaultTestFolderPath: string = path.join(
      projectPath,
      "src",
      "test",
      "java",
    ...
    const newTestFolderPath: string = path.join(
      projectPath,
      "src",
      "test",
      "java",
...
const deleteDefaultFolder = (
  ...
  if (groupId === "it.theapplegeek") {
    ...
    deleteFolder(
      path.join(projectPath, "src", "main", "java", "it", "theapplegeek"),
    );
    deleteFolder(
      path.join(projectPath, "src", "test", "java", "it", "theapplegeek"),
    );
  } else {
